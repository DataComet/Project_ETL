...
def harmonize_weather_data():
    ...
    json_file = open(CURR_DIR_PATH + "/raw_json_data.json", 'r')
...
def format_harmonized_data():
    json_file = open(CURR_DIR_PATH + "/harmonized_json_data.json", 'r')
    harmonized_data = json.load(json_file)
    df = pd.DataFrame.from_dict(harmonized_data)
    
    df.to_csv("formated_data.csv",index=False)

with DAG("weather_forecast", start_date=dt(2022, 7, 13),
    ...
    harmonize_task = PythonOperator(
        task_id="harmonize_raw_data",
        python_callable=harmonize_weather_data

    )
    
    format_task = PythonOperator(
        task_id="format_harmonized_data",
